...
import { LayoutDashboard, Ticket, Users, UserCog, Star, Settings, Briefcase, CreditCard, BarChart3, Plus, MessageSquare, ChevronLeft, ChevronRight, Menu, LogOut, User, ChevronDown, Mail } from 'lucide-react';
import Sidebar from './Sidebar';
import AdminFooter from './AdminFooter';
import { ThemeToggle, ServiceProvider, ServiceDrawer, StaffProvider, StaffDrawer, TicketProvider, TicketDrawer, useStaff } from '../shared';
import { useAuth } from '../../context/AuthContext';
import toast from 'react-hot-toast';

const BACKEND_URL = import.meta.env.VITE_API_URL?.replace('/api', '') || 'http://localhost:5000';

const CompanyHeader = ({ slug, collapsed, setCollapsed, mobileOpen, setMobileOpen, user }) => {
  ...
  return (
    <header className="topbar">
      <div className="topbar-left">
        ...
        <span className="topbar-title">{user?.company?.name || 'Company'} — Support</span>
      </div>
      <div className="topbar-right">
        <ThemeToggle/>
        <button
          onClick={()=>staffContext?.setStaffDrawerOpen?.(true)}
          style={{
            display:'flex',
            alignItems:'center',
        ...
        <div ref={profileDropdownRef} style={{ position:'relative', marginLeft:8 }}>
          ...
          {profileDropdownOpen && (
            <div style={{
              ...
              <div style={{ padding:6 }}>
                <button
                  onClick={()=>{ navigate(`/${slug}/profile`); setProfileDropdownOpen(false); }}
                  style={{
                    width:'100%',
                    display:'flex',
                    alignItems:'center',
                ...
                <button
                  onClick={()=>{ navigate(`/${slug}/settings`); setProfileDropdownOpen(false); }}
                  style={{
                    width:'100%',
                    display:'flex',
                    alignItems:'center',
  ...
  const navSections = [
    {
      label: 'Overview',
      items: [
        { to:`/${slug}/dashboard`, icon:<LayoutDashboard size={17}/>, label:'Dashboard', end:true },
        { to:`/${slug}/analytics`, icon:<BarChart3 size={17}/>,       label:'Analytics & Reports' },
      ],
    },
    {
      label: 'Support',
      items: [
        { to:`/${slug}/tickets`, icon:<Ticket size={17}/>,    label:'Tickets' },
        { to:`/${slug}/users`,   icon:<Users size={17}/>,     label:'Customers' },
      ],
    },
    isAdmin ? {
      label: 'Team',
      items: [
        { to:`/${slug}/staff`,    icon:<UserCog size={17}/>,   label:'Staff & Agents' },
        { to:`/${slug}/services`, icon:<Briefcase size={17}/>, label:'Services' },
      ],
    } : null,
    {
      label: 'Feedback',
      items: [
        { to:`/${slug}/ratings`, icon:<Star size={17}/>, label:'Ratings & Feedback' },
      ],
    },
    isAdmin ? {
      label: 'Account',
      items: [
        { to:`/${slug}/subscription`, icon:<CreditCard size={17}/>, label:'Subscription' },
        { to:`/${slug}/settings`,     icon:<Settings size={17}/>,   label:'Settings' },
        { to:`/${slug}/profile`,      icon:<User size={17}/>,       label:'My Profile' },
      ],
    } : null,
  ].filter(Boolean);
        ...
        <TicketProvider>
          <div className="app-layout">
            ...
            <Sidebar
              ...
              brandName={user?.company?.name || 'Company'}
              brandTagline={`/${slug}`}
              brandLogo={user?.company?.branding?.logo}
              collapsed={collapsed}
