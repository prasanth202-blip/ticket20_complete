...
import { LayoutDashboard, Ticket, MessageSquare, Star, User, ChevronLeft, ChevronRight, Menu, LogOut, ChevronDown, Mail, Plus } from 'lucide-react';
...
const AgentHeader = ({ slug, collapsed, setCollapsed, mobileOpen, setMobileOpen }) => {
  ...
  return (
    <header className="topbar">
      <div className="topbar-left">
        ...
        <span className="topbar-title">Agent — {user?.company?.name}</span>
      </div>
        ...
        <div ref={profileDropdownRef} style={{ position:'relative', marginLeft:8 }}>
          ...
          {profileDropdownOpen && (
            <div style={{
              ...
              <div style={{ padding:6 }}>
                <button
                  onClick={()=>{ navigate(`/${slug}/agent/profile`); setProfileDropdownOpen(false); }}
                  style={{
                    width:'100%',
                    display:'flex',
                    alignItems:'center',
...
export default function AgentLayout() {
  ...
  const navSections = [{
    label: 'Agent Portal',
    items: [
      { to:`/${slug}/agent/dashboard`, icon:<LayoutDashboard size={17}/>, label:'Dashboard', end:true },
      { to:`/${slug}/agent/tickets`,   icon:<Ticket size={17}/>,          label:'My Tickets' },
      { to:`/${slug}/agent/profile`,   icon:<User size={17}/>,            label:'My Profile' },
    ],
  }];
  ...
  return (
    <TicketProvider>
      <div className="app-layout">
        ...
        `}</style>
        <Sidebar navSections={navSections} brandName={user?.company?.name} brandTagline="Agent Portal" brandLogo={user?.company?.branding?.logo} collapsed={collapsed} onToggleCollapse={()=>setCollapsed(!collapsed)} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen}/>
